...
def smallest_array
  ...
  jrn1 = jrn.sort_by {|jrn| jrn.length}
  puts "le plus petit array est #{jrn1[0]}"
end
puts smallest_array

#NOMBRE D'HANDLE CONTENANT 5 CARACTERES
def five_caracter
  handle5 = jrn.find_all { |l| l.size == 6 }
...
def maj
  ...
  jrn.each do |handle|
    ...
    # or count += 1 if handle[1] == handle[1].upcase
  end
end
puts maj


#HANDLE EN ORDRE ALPHABETIC
def alpha_order
  puts "Array par ordre alphabétic : #{jrn.sort}"
end
puts alpha_order
...
#POSITION DE @EPENSER
def position
  puts "@epenser est en #{jrn.find_index("@epenser")} ieme position"
end
puts position
